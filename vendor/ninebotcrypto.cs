/*
This file is used for reference on Ninebot BLE protocol encryption
Origin: https://github.com/scooterhacking/NinebotCrypto
*/
using System;
...
namespace es2_Flasher
{
    class NinebotCrypto
    {
        ...
        private byte[] this.shaKey = new byte[16];
        ...
        private byte[] CryptoFirst(byte[] Data)
        {
            ...
            while (payload_len > 0)
            {
                ...
                byte[] aes_key = AesEcbEncrypt(_fw_data, this.shaKey);
        ...
        private byte[] CryptoNext(byte[] Data, UInt32 MsgIt)
        {
            ...
            while (payload_len > 0)
            {
                ...
                byte[] aes_key = AesEcbEncrypt(aes_enc_data, this.shaKey);
                Buffer.BlockCopy(aes_key, 0, xor_data_2, 0, 16);
                byte[] xor_data = XOR(xor_data_1, xor_data_2, 16);
        ...
        private byte[] CalcCrcNextMsg(byte[] Data, UInt32 MsgIt)
        {
            ...
            aes_key = AesEcbEncrypt(aes_enc_data, this.shaKey);
            Buffer.BlockCopy(aes_key, 0, xor_data_2, 0, 16);
            ...
            aes_key = AesEcbEncrypt(xor_data, this.shaKey);
            Buffer.BlockCopy(aes_key, 0, xor_data_2, 0, 16);
        ...
        private void CalcSha1Key(byte[] sha1_data_1, byte[] sha1_data_2)
        {
            ...
            Buffer.BlockCopy(sha_hash, 0, this.shaKey, 0, 16);
        }
